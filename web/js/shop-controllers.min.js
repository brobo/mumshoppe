...
'AccessoryService',
'CategoryService',
function($scope, promiseTracker, AlertService, AccessoryService, CategoryService) {
	...
	$scope.mumPromise.then(AccessoryService.findAll).then(function(data) {
	...
	$scope.hasAccessory = function(accessory) {
		for (var i = 0; i < $scope.mum.accessories.length; i++) {
			if ($scope.mum.accessories[i].id == accessory.id) {
				return true;
			}
		}
		return false;
	...
	$scope.addAccessory = function(accessory) {
		...
		if ($scope.mum.accessories.length >= $scope.mum.backing.product.accessoryLimit) return;
		var target = angular.copy($scope.mum);
		for (var i = 0; i < target.accessories.length; i++) {
			target.accessories[i] = target.accessories[i].id;
		}
		target.accessories.push(accessory.id);

		$scope.save(target).finally(deferred.resolve);
	}
	
	$scope.removeAccessory = function(accessory) {
		var deferred = accessory.tracker.createPromise();

		var target = angular.copy($scope.mum);
		for (var i = 0; i < target.accessories.length; i++) {
			if (target.accessories[i].id == accessory.id) {
				target.accessories.splice(i, 1);
				i--;
			} else {
				target.accessories[i] = target.accessories[i].id;
			}
